...
// Bezpieczne zwalnianie pamięci, aby uniknąć wskaźników wiszących
...
// Sprawdza, czy linia zawiera poprawne żądanie konwersji między bazami
...
// Sprawdza, czy linia zawiera poprawne żądanie operacji arytmetycznej
...
// Główna funkcja do parsowania danych wejściowych i wykonywania operacji
void parse_and_execute(FILE* input, FILE* output)
{
    ...
    while (fgets(line, sizeof(line), input)) {
        ...
        // Włacznie flagi is_base_conversion_mode jeśli występuję konwersja
        if ((is_base_conversion_request(line, &base1, &base2) || (start_of_file && is_base_conversion_mode)))
        {
            ...
            operation_started = 1;
            fprintf(output, "%d %d\n\n", base1, base2);
            // Sprawdzanie poprawności zmiennych
        ...
        else if (is_arithmetic_operation_request(line, &operation, &base) || (start_of_file && operation))
        {
            ...
            while (fgets(line, sizeof(line), input))
            {
                ...
                // jesli linia jest pusta, powiększanie wartości empty_line_count
                if (strlen(line) == 0)
                {
                    empty_line_count++;
                    if (empty_line_count == 3)
                ...
                else
                {
                    ...
                    // Dokonywanie odpowiedniej operacji w zależności od znaku
                    switch (operation)
                    {
                    case '+':
...
// Konwertuje liczbę z systemu dziesiętnego na dowolny system liczbowy
...
//Główna funkcja obsługująca otwieranie i zamykanie plików txt oraz uruchamiająca funkcję parse_and_execute
