...
// Funkcja do dynamicznego odczytu linii o dowolnej d ugo ci
...
void parse_and_execute(FILE* input, FILE* output)
{
    ...
    char* line = NULL; // Wska nik do dynamicznie alokowanej linii
    ...
    while ((line = read_line(input)) != NULL) {
        ...
        if (strlen(line) == 0)
        {
            ...
            if (empty_line_count == 3)
            {
                ...
                operation_started = 0;
                free(line); // Zwolnienie pami ci po zako czeniu przetwarzania linii
                continue;
            }
            free(line); // Zwolnienie pami ci po zako czeniu przetwarzania linii
            continue;
        ...
        else if (is_arithmetic_operation_request(line, &operation, &base) || (start_of_file && operation))
        {
            ...
            while ((line = read_line(input)) != NULL)
            {
                ...
                // Je li linia jest pusta, powi kszanie warto ci empty_line_count
                if (strlen(line) == 0)
                {
                    empty_line_count++;
                    if (empty_line_count == 3)
...
char* decimal_to_base(const char* num, int base_to)
{
    ...
    while (compare_strings(quotient, "0", 10) > 0)
    {
        ...
        char rem_char = (*remainder >= '0' && *remainder <= '9') ? *remainder : *remainder - 'A' + 10 + 'A';
